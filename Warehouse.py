...
def init_db():
    ...
    # جدول الأصناف (تم التأكد من تطابق الحقول)
    cursor.execute('''
...
with st.sidebar:
    ...
    if user['role'] in ['Admin', 'Storekeeper']:
        if st.button("🏬 إضافة/تعديل مخزن فرعي", use_container_width=True): st.session_state['menu'] = "المخازن"
        if st.button("🏷️ تكويد صنف جديد", use_container_width=True): st.session_state['menu'] = "الأصناف"
        if st.button("🔄 حركة مخزنية (إضافة/صرف/مرتجع)", use_container_width=True): st.session_state['menu'] = "الحركات المخزنية"
    if st.button("📊 جرد ورصيد المخزون الحالي", use_container_width=True): st.session_state['menu'] = "جرد المخزون"
    ...
    if user['role'] in ['Admin', 'Sales', 'Accountant', 'Storekeeper']:
        if st.button("🤝 تكويد وتعديل عميل / مورد", use_container_width=True): st.session_state['menu'] = "الشركاء"

    st.markdown('<div class="sidebar-category">🧾 المبيعات والفواتير</div>', unsafe_allow_html=True)
    if user['role'] in ['Admin', 'Sales', 'Accountant', 'Storekeeper']:
        if st.button("📄 إصدار فاتورة مبيعات", use_container_width=True): st.session_state['menu'] = "إصدار فاتورة"
        if st.button("🔍 استعلام عن الفواتير", use_container_width=True): st.session_state['menu'] = "استعلام الفواتير"

    st.markdown('<div class="sidebar-category">💰 الحسابات والمالية والخزنة</div>', unsafe_allow_html=True)
    if user['role'] in ['Admin', 'Accountant']:
        if st.button("💵 الخزنة واليومية (وارد/منصرف)", use_container_width=True): st.session_state['menu'] = "الخزنة والماليات"
        if st.button("🏦 البنوك والشيكات", use_container_width=True): st.session_state['menu'] = "البنوك والشيكات"

    st.markdown('<div class="sidebar-category">👔 الموارد البشرية HR</div>', unsafe_allow_html=True)
    if user['role'] in ['Admin', 'HR', 'Accountant']:
        if st.button("📋 الموظفين والرواتب والسلف", use_container_width=True): st.session_state['menu'] = "الموارد البشرية"

    st.markdown('<div class="sidebar-category">📈 التقارير والإحصائيات</div>', unsafe_allow_html=True)
    if st.button("📊 التقارير الشاملة والبيانات", use_container_width=True): st.session_state['menu'] = "التقارير"
    if user['role'] == 'Admin':
        if st.button("👑 تقارير الإدارة العليا (صاحب الشركة)", use_container_width=True): st.session_state['menu'] = "تقارير الإدارة العليا"

    if user['role'] == 'Admin':
        st.markdown('<div class="sidebar-category">⚙️ الإعدادات والصلاحيات</div>', unsafe_allow_html=True)
        if st.button("👤 إدارة المستخدمين وكلمات السر", use_container_width=True): st.session_state['menu'] = "المستخدمين"
        if st.button("💾 النسخ الاحتياطي (Backup/Restore)", use_container_width=True): st.session_state['menu'] = "النسخ الاحتياطي"

menu_choice = st.session_state['menu']
...
if menu_choice == "المخازن":
    ...
    with st.form("form_add_warehouse"):
        ...
        if submit_w:
            if w_name.strip() != "":
                ...
                try:
                    ...
                    st.error("❌ اسم المخزن موجود مسبقاً! اختر اسماً آخر.")
    ...
    st.subheader("📋 قائمة المخازن المسجلة حالياً")
    conn = get_db_connection()
    df_w = pd.read_sql("SELECT id as 'م', name as 'اسم المخزن', location as 'الموقع', manager as 'المسؤول' FROM warehouses", conn)
    conn.close()
    
    if df_w.empty:
        st.info("💡 لا يوجد مخازن مسجلة حتى الآن.")
    else:
        st.dataframe(df_w, use_container_width=True)


# ------------------------------------------
# 2. تكويد صنف جديد (تم تعديل المعالجة لضمان الحفظ)
# ------------------------------------------
elif menu_choice == "الأصناف":
    st.title("🏷️ تكويد صنف جديد والتعديل")
    
    with st.form("form_add_item_new"):
        ...
        with c3:
            min_quantity = st.number_input("حد الأمان (الحد الأدنى)", min_value=0, value=5)
            cost_price = st.number_input("سعر التكلفة / השراء", min_value=0.0, value=0.0)
            selling_price = st.number_input("سعر البيع", min_value=0.0, value=0.0)
        
        description = st.text_area("وصف إضافي وملاحظات الصنف")
        submit_item = st.form_submit_button("💾 حفظ الصنف الآن")
            ...
            if item_code.strip() != "" and name.strip() != "":
                ...
                try:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (item_code.strip(), name.strip(), category, serial_number, part_number, int(min_quantity), unit, float(cost_price), float(selling_price), description))
                    conn.commit()
                    st.success(f"✅ تم حفظ الصنف '{name}' بنجاح في قاعدة البيانات!")
                except sqlite3.IntegrityError:
                    st.error("❌ كود الصنف هذا مسجل مسبقاً! يرجى تغيير الكود.")
                except Exception as e:
    ...
    st.subheader("📋 قائمة الأصناف المسجلة في قاعدة البيانات")
    conn = get_db_connection()
    df_items = pd.read_sql("SELECT id as 'م', item_code as 'كود الصنف', name as 'اسم الصنف', category as 'التصنيف', min_quantity as 'حد الأمان', cost_price as 'سعر الشراء', selling_price as 'سعر البيع' FROM items", conn)
    conn.close()
...
# ------------------------------------------
    ...
    if warehouses.empty and items.empty:
        st.warning("⚠️ يجب عليك إضافة (مخزن) من شاشة المخازن و (صنف) من شاشة الأصناف أولاً.")
    elif warehouses.empty:
        st.warning("⚠️ لا يوجد أي مخزن مسجل! اضغط على 'إضافة/تعديل مخزن فرعي' أولاً.")
    elif items.empty:
        st.warning("⚠️ لا يوجد أي صنف مسجل! اضغط على 'تكويد صنف جديد' وأضف صنفك الأول.")
    else:
        with st.form("form_transaction"):
            ...
            with c1:
                ...
                selected_item_name = st.selectbox("اختر الصنف", items['name'].tolist())
                
            with c2:
                if trans_type == "تحويل بين المخازن":
                    from_wh = st.selectbox("من مخزن (صرف)", warehouses['name'].tolist(), key='f_wh')
                    to_wh = st.selectbox("إلى مخزن (إضافة)", [w for w in warehouses['name'].tolist() if w != from_wh], key='t_wh')
                else:
                    selected_wh_name = st.selectbox("المخزن المعني", warehouses['name'].tolist())
                    partner_list = ["بدون"] + partners['name'].tolist() if not partners.empty else ["بدون"]
                    selected_partner_name = st.selectbox("العميل / المورد المعني", partner_list)

            c3, c4 = st.columns(2)
            with c3:
                quantity = st.number_input("الكمية", min_value=1, value=1)
                unit_price = st.number_input("السعر للوحدة", min_value=0.0, value=0.0)
            with c4:
                person_in_charge = st.text_input("المسؤول / المستلم")
                notes = st.text_area("ملاحظات الحركة")

            submit_trans = st.form_submit_button("💾 تسجيل الحركة المخزنية")

            if submit_trans:
                conn = get_db_connection()
                curr = conn.cursor()
                
                item_id = int(items[items['name'] == selected_item_name]['id'].values[0])
                
                p_id = None
                if trans_type != "تحويل بين المخازن" and not partners.empty and selected_partner_name != "بدون":
                    p_id = int(partners[partners['name'] == selected_partner_name]['id'].values[0])
                ...
                if trans_type == "تحويل بين المخازن":
                    ...
                    curr.execute("""
                        ...
                        VALUES (?, 'صرف', ?, ?, ?, ?, ?, ?)
                    """, (str(trans_date), f_id, item_id, quantity, unit_price, person_in_charge, f"تحويل إلى {to_wh}: {notes}"))
                    
                    curr.execute("""
                        INSERT INTO inventory_transactions (date, trans_type, warehouse_id, item_id, quantity, unit_price, person_in_charge, notes)
                        VALUES (?, 'إضافة', ?, ?, ?, ?, ?, ?)
                    """, (str(trans_date), t_id, item_id, quantity, unit_price, person_in_charge, f"تحويل من {from_wh}: {notes}"))
                    st.success(f"✅ تم تحويل الكمية ({quantity}) بنجاح!")
                else:
                    w_id = int(warehouses[warehouses['name'] == selected_wh_name]['id'].values[0])
...
# ------------------------------------------
    ...
    if not df_stock.empty:
        ...
        st.subheader("جدول جرد المخزون التفصيلي")
        st.dataframe(df_stock, use_container_width=True)
        st.metric("إجمالي قيمة البضاعة بالمخازن", f"{df_stock['إجمالي القيمة'].sum():,.2f} ج.م")
    else:
        st.info("💡 لا توجد حركات مخزنية أو أرصدة مسجلة حتى الآن. (قم بتسجيل حركة إضافة من شاشة الحركات المخزنية).")
...
# ------------------------------------------
    ...
    conn = get_db_connection()
    df_p = pd.read_sql("SELECT id as 'م', name as 'الاسم', type as 'النوع', phone as 'الهاتف', address as 'العنوان' FROM partners", conn)
    conn.close()
...
# ------------------------------------------
    ...
    if partners.empty or items.empty or warehouses.empty:
        st.warning("⚠️ يجب توفر عميل ومخزن وصنف واحد على الأقل لإصدار الفاتورة.")
    else:
        with st.form("invoice_form"):
            ...
            if st.form_submit_button("🪶 إصدار وطباعة الفاتورة"):
                conn = get_db_connection()
                curr = conn.cursor()
                ...
                st.success("✅ تم إصدار الفاتورة وخصم الكمية وتسجيل الإيراد بنجاح!")
...
# ------------------------------------------
    ...
    conn = get_db_connection()
    df_t = pd.read_sql("SELECT id as 'م', date as 'التاريخ', trans_type as 'النوع', category as 'البند', amount as 'المبلغ', statement as 'البيان' FROM treasury ORDER BY id DESC", conn)
    conn.close()
    
    if not df_t.empty:
        total_in = df_t[df_t['النوع'] == 'إيراد / وارد']['المبلغ'].sum()
        total_out = df_t[df_t['النوع'] == 'مصروف / منصرف']['المبلغ'].sum()
...
# ------------------------------------------
    ...
    with st.form("cheque_form"):
        ...
        if st.form_submit_button("💾 حفظ الشيك"):
            conn = get_db_connection()
            p_id = int(partners[partners['name'] == partner_sel]['id'].values[0]) if not partners.empty and partner_sel != "بدون" else None
...
# ------------------------------------------
    ...
    with tab2:
        ...
        if not emps.empty:
            with st.form("emp_adv_form"):
                ...
                if st.form_submit_button("💾 تسجيل السلفة وخصمها من الخزنة"):
                    conn = get_db_connection()
    ...
    conn = get_db_connection()
    df_e = pd.read_sql("SELECT id as 'م', emp_name as 'اسم الموظف', position as 'الوظيفة', basic_salary as 'الراتب الأساسي', advances as 'إجمالي السلف' FROM hr_employees", conn)
    conn.close()
...
# ------------------------------------------
    ...
    conn = get_db_connection()
    st.subheader("📊 رسم بياني للحركات المخزنية")
    try:
    ...
    st.subheader("سجل الحركات المخزنية")
...
# ------------------------------------------
    ...
    try:
        ...
        st.info("لا توجد فواتير كافية للعرض.")
    conn.close()
...
# ------------------------------------------
    ...
    conn = get_db_connection()
    df_u = pd.read_sql("SELECT id as 'م', username as 'اسم المستخدم', full_name as 'الاسم', role as 'الصلاحية' FROM users", conn)
    conn.close()
